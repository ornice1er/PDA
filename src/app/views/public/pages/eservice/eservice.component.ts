...
export class EserviceComponent implements OnInit {
  data: any[] = [];
  textFileIcon = FileText;
  private wowSubscription: Subscription | undefined;

  constructor(
    private router: Router,
    private pdaService: PdaService //  private wowService: NgwWowService
  ) {
  ...
  filterContactPoints() {
    this.filteredContactPoints = this.contactPoints.filter((point) => {
      const matchesSearch =
        ...
        point.desc.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
        point.slug.toLowerCase().includes(this.searchTerm.toLowerCase());
  ...
  truncatedText(text: string): string {
    const words = text.split(/\s+/); // découpe la chaîne en mots
