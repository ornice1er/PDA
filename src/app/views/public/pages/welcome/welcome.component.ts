...
import { HeroComponent } from '../../public-layout/components/hero/hero.component';
import { ServicesComponent } from '../../public-layout/components/services/services.component';
import { StatisticsComponent } from '../../public-layout/components/statistics/statistics.component';
...
export class WelcomeComponent {
  ...
  loadServices(event: any) {
    this.default_services = this.e_services.filter(
      (e: any) => e.key == event.target.value
    )[0];
  }
  ...
  confirmation(service: any) {
    Swal.fire({
      text:
        'Inscrivez-vous et authentifiez-vous pour acceder au service ' +
        service,
      icon: 'warning',
  ...
  constructor(
    ...
    if (
      this.route.snapshot.paramMap.get('client_id') &&
      this.route.snapshot.paramMap.get('client_secret')
    ) {
      this.visitor = true;
  ...
  ngOnInit(): void {
    ...
    this.pdaService.getFaqs().subscribe((res: any) => {
        ...
        if (res.check_code) {
          ...
          if (res.user.active) {
            ...
            if (res.user.is_portal_admin == true) {
              ...
              this.local_service.set(
                GlobalName.refresh_token,
                res.refresh_token
              );
