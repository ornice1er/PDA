...
export class HomepfcComponent implements OnInit {
  ...
  addRequeteusager(value:any){
    ...
    if (this.selected_type_preoccupation == 0) {
      AppSweetAlert.simpleAlert("error","error","Sélectionner le type de requête", "Champ obligatoire")
    }else if(!value.idEntite){
      AppSweetAlert.simpleAlert("error","error","Sélectionner la structure destinatrice", "Champ obligatoire")
    }else if(!value.contactUs){
      AppSweetAlert.simpleAlert("error","error","Renseigner le contact", "Champ obligatoire")
    }else if(this.mat_aff == true && value.matricule == ''){
      AppSweetAlert.simpleAlert("error","error","Renseigner le matricule", "Champ obligatoire")
    }else if((value.idPrestation == "" || value.idPrestation == null) && this.link_to_prestation==1){
      AppSweetAlert.simpleAlert("error","error","Sélectionner la prestation", "Champ obligatoire")
    }else if(!value.objet){
      AppSweetAlert.simpleAlert("error","error","Renseigner l'objet", "Champ obligatoire")
    }else if(!value.msgrequest){
      AppSweetAlert.simpleAlert("error","error","Renseigner le message", "Champ obligatoire")
    }else{
      let interfa = null
      let idReg = null
      let idEsWha = null
  ...
  addReponse(value:any){
    ...
    if (value.reponse_whats == "") {
      AppSweetAlert.simpleAlert("error","error","Renseigner la réponse à cette discusion", "Champ obligatoire")
      return;
    }else{
  ...
  saveRequeteusager(value:any) {
    ...
    if (this.selected_type_preoccupation == 0) {
      AppSweetAlert.simpleAlert("error","Sélectionner le type de requête", "Champ obligatoire")
    }else if(!value.idEntite2){
      AppSweetAlert.simpleAlert("error","Sélectionner la structure destinatrice", "Champ obligatoire")
    }else if(!value.contactUs2){
      AppSweetAlert.simpleAlert("error","Renseigner le contact", "Champ obligatoire")
      ...
      AppSweetAlert.simpleAlert("error","Sélectionner la prestation", "Champ obligatoire")
  ...
  addRequeteRv(value:any){
    if (value.contactMatri == "") {
      AppSweetAlert.simpleAlert("error","Renseigner le matricule ou le contact du visiteur", "Champ obligatoire")
    }else if(!value.plainterv){
      AppSweetAlert.simpleAlert("success","error","Sélectionner le type", "Champ obligatoire")
    }else if(value.nom_pre_rv == ""){
      AppSweetAlert.simpleAlert("error","Renseigner le nom et prénom (s) du visiteur", "Champ obligatoire")
    }else if(!value.idEntite){
      AppSweetAlert.simpleAlert("success","error","Sélectionner la structure destinatrice", "Champ obligatoire")
    }else if((value.preoccurv == "" || value.preoccurv == null)){
      AppSweetAlert.simpleAlert("error","Renseigner la préoccupation du visiteur", "Champ obligatoire")
    }else if(value.satisfaitrv == ""){
      AppSweetAlert.simpleAlert("error","Sélectionner une appréciation", "Champ obligatoire")
    }else if(value.satisfaitrv && value.observarv == ""){
      AppSweetAlert.simpleAlert("error","Renseigner l'observation", "Champ obligatoire")
    }else{
      var param = {
  ...
  addWhatsApp(value:any){
    if (value.contWhatsapp == "") {
      AppSweetAlert.simpleAlert("error","Renseigner le contact WhatsApp", "Champ obligatoire")
    }else if(value.discussiontxt == ""){
      AppSweetAlert.simpleAlert("error","Renseigner la discussion", "Champ obligatoire")
    }else{
      var param = {
  ...
  UpdateRequeteRv(value:any){
    ...
    if (value.contactMatri == "") {
      ...
      AppSweetAlert.simpleAlert("error","Sélectionner le type", "Champ obligatoire")
    }else if(value.nom_pre_rv == ""){
      AppSweetAlert.simpleAlert("error","Renseigner le nom et prénom (s) du visiteur", "Champ obligatoire")
    }else if(!value.idEntite){
      AppSweetAlert.simpleAlert("error","Sélectionner la structure destinatrice", "Champ obligatoire")
  ...
  this.user_auth_service.getAllForRegistreVis(this.user.id, 1).subscribe((res: any) => {
    ...
    gotoHashtag(fragment: string) {
      ...
      if(this.nbreDay.length != 0){
        AppSweetAlert.simpleAlert('info',"Date non clôturée","Vous aviez '+this.nbreDay.length+' date(s) non cloturée(s).")
        setTimeout(function () {
    ...
    "Cette action est irreversible. Voulez-vous continuer ?").then((result:any) => {
      if (result.value) {
        this.user_auth_service.deleteRegistreVis(this.selected_data_reqrv.id).subscribe((res: any) => {
          ...
          AppSweetAlert.simpleAlert("error","Suppression visite", "Erreur, Verifiez que vous avez une bonne connexion internet")
        })
      }
    })
}
  ...
  AppSweetAlert.confirmBox("Suppression cette discussion", "Cette action est irreversible. Voulez-vous continuer ?").then((result:any) => {
      if (result.value){
        this.user_auth_service.deleteDiscWhats(this.selected_data_Whats.id).subscribe((res: any) => {
          ...
          AppSweetAlert.simpleAlert("error","Suppression discussions", "Erreur, Verifiez que vous avez une bonne connexion internet")
        })
      }
    })
}
  ...
  AppSweetAlert.confirmBox("Confirmer l'ajout", "Souhaitez-vous ajouter cette discussion dans mataccueil ?").then((result:any) => {
      if (result.value){
        this.user_auth_service.ConfirmerDiscWhats(this.user.id, el.id).subscribe((res: any) => {
          ...
          AppSweetAlert.simpleAlert("error","Confirmer l'ajout", "Erreur, Verifiez que vous avez une bonne connexion internet")
        })
      }
    })
}
